...
var db = monk(
  'localhost:27017/users'
); /*we are creating a database with name users. here 27017 is default mongodb port */
var collection = db.get('logindetails');
var posdet = db.get(
  'projdetails'
); /* we created a collection in the database named logindetails */
/* GET home page. */
...
router.get('/myprojs', function(req, res) {
  if (req.session && req.session.usable) {
    res.locals.usable = req.session.usable;
    posdet.find({ leadername: req.session.usable.Username }, function(
      err,
      docs
    ) {
      res.render('myprojects', { project: docs });
    });
  }
});
